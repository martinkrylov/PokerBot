...
class HoldemTable(Env):
    ...
    def _next_player(self):
        ...
        for player in range(len(self.player_cycle.alive)):
            if self.players[player].name == "neo" and not self.player_cycle.alive[player]:
                log.info("End round - our player busted")
                self.reset()
                return
